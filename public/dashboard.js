...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (confirmBtn) {
        confirmBtn.addEventListener('click', async () => {
            ...
            if (currentAction === "deposit") {
                ...
                try {
                    const response = await fetch(`/api./tinubu_users`);
                    ...
                    document.getElementById('final-confirm-btn').onclick = async () => {
                        ...
                        if (myAccount.transfer(amount, recipientUser.name)) {
                            ...
                            await fetch(`/api./tinubu_users/${recipientUser.id}`, {
                                method: 'PATCH',
    ...
    if (savePinBtn) {
        savePinBtn.addEventListener('click', async () => {
            ...
            try {
                await fetch(`/api./tinubu_users/${myAccount.id}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
...
async function syncDatabase(updatedAccount) {
    try {
        await fetch(`/api./tinubu_users/${updatedAccount.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
